...
import { fetchTrainingSeriesById, fetchTrainingSeries } from "@/lib/utils/Training/TrainingSeriesUtils";
...
export default function WorkshopDetail() {
  ...
  const [training, setTraining] = useState<TrainingSeriesItem | null>(null);
  const [allSeries, setAllSeries] = useState<TrainingSeriesItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const [current, all] = await Promise.all([
        fetchTrainingSeriesById(Number(id)),
        fetchTrainingSeries(),
      ]);
      setTraining(current);
      setAllSeries(all);
  ...
  const currentIndex = allSeries.findIndex((t) => t.id === Number(id));
  const prevTraining = allSeries[currentIndex - 1];
  const nextTraining = allSeries[currentIndex + 1];

  return (
    <div className="w-full min-h-screen bg-cover bg-center bg-no-repeat relative p-10">
      <img
      ...
      <div className="flex flex-col relative z-10 mx-auto py-10 gap-10">
        ...
        <section className="mb-8 text-center">
          ...
          <ul className="inline-block text-left text-md md:text-lg text-gray-800 space-y-1">
            {training.who_is_this_for.map((item, i) => (
