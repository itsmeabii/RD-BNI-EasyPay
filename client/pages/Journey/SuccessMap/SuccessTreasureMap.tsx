...
import { TrainingSeriesItem } from "@/types/JourneyTypes";
import { fetchTrainingSeries } from "@/lib/utils/Training/TrainingSeriesUtils";

const PIN_COLORS = [
  "#CF2031", "#E8734A", "#4A90D9", "#5BAD6F",
  "#7B5EA7", "#D94FAB", "#E8A020", "#3ABFBF",
];
...
function TreasurePin({ number, color, label, left, top, onClick }: {
  number: number;
  color: string;
  label: string;
  left: string;
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center flex-shrink-0">
        <svg width="32" height="43" viewBox="0 0 28 38" fill="none" className="drop-shadow-lg transition-all">
          ...
          <text x="14" y="18" textAnchor="middle" fill="white" fontSize="10" fontWeight="bold">{number}</text>
        </svg>
      </div>
      <div className="mt-1">
        <p className="text-[10px] md:text-[12px] font-bold leading-tight max-w-[120px] uppercase"
          style={{ color, textShadow: "0 1px 3px rgba(255,255,255,0.9)" }}>
          {label}
        </p>
      </div>
    </div>
...
export default function SuccessTreasureMap() {
  ...
  const [series, setSeries] = useState<TrainingSeriesItem[]>([]);

  useEffect(() => {
    fetchTrainingSeries().then(setSeries);
  }, []);
    ...
    <div className="px-4 md:px-20 bg-white min-h-screen py-8">
      <div className="w-full aspect-[16/9] md:aspect-[1073/598] relative rounded-lg overflow-hidden">
        ...
        <div
          ...
          style={{ backgroundImage: "url('https://api.builder.io/api/v1/image/assets/TEMP/56747f665c7ffb39e050d8f6bcf34d3cda136a5e?width=2127')" }}
        />
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1000 560" preserveAspectRatio="xMidYMid slice">
          <path
            ...
            fill="none" stroke="#777" strokeWidth="4" strokeDasharray="14 9"
            strokeLinecap="round" strokeLinejoin="round"
          />
        </svg>

        {/* Training series pins */}
        {series.map((s, i) => (
          <TreasurePin
            key={s.id}
            number={i + 1}
            ...
            onClick={() => navigate(`/success-treasure-map/${s.id}?month=${monthParam}`)}
          />
        ))}
      </div>
    </div>
