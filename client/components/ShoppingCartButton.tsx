...
export default function ShoppingCartButton() {
  const { totalCount, openCart, isCartOpen, isLoading } = useCart();
  ...
  return (
    <button 
      ...
      {!isLoading && totalCount > 0 && ( 
        <span className="fixed -top-1 -left-1 bg-bni-red text-bni-white rounded-full w-4 h-4 lg:w-6 lg:h-6 flex items-center justify-center text-xs font-bold">
          {totalCount}
        </span>
      )}
    </button>
